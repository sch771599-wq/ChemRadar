...
# ========== ФУНКЦИЯ ДЛЯ ЛЮБЫХ SMILES ==========
def safe_smiles_to_mol(smiles):
    """Convert SMILES to molecule - handles aromaticity errors"""
    ...
    # Method 2: parse without sanitization
    try:
    ...
    # Method 3: remove numbers after atoms
...
def build_search_urls(name, keywords, cid, ikey, smiles):
    ...
    if search_terms:
        ...
        # For regular Google - only first word, no filter
...
# ========== ПОИСК АНАЛОГОВ С НЕСКОЛЬКИМИ ССЫЛКАМИ ==========
def find_and_show_analogs(cid, smiles, name, keywords, ikey):
    """Finds similar compounds and shows multiple links for each"""
    ...
    # Try to find analogs via PubChem API
    analogs_list = []
    try:
            ...
            with cols[idx]:
                try:
                    ...
                    if analog_mol:
                        ...
                        # Multiple links for each analog
                        st.markdown("**Links:**")
                    ...
                    st.warning(f"Could not load analog {idx+1}")
    else:
        # If no analogs found via API, show alternative search links
...
with left_col:
    ...
    # Submit button for convenience (Enter doesn't work in Streamlit text_area)
...
with right_col:
    if should_process or (user_smiles and st.session_state.get('processed', False)):
        ...
        if mol:
            ...
            # Search for similar compounds with multiple links
